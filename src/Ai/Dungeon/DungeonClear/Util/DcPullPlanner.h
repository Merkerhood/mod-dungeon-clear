...
public:
    ...
    // a point `setback` back along the breadcrumb trail behind the tank, so the
    // camp can TRAIL the moving tank while no pull is committed and the party
    // walks along behind it at a fixed standoff. (`maxDrag` is accepted but has
    // never bounded this walk — see the note in the implementation.)
    // No clearance test —
